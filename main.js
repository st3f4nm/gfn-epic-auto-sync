...
const clickEpicGamesStoreOption = async () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const epicStoreBtn =
      document.querySelector('button[aria-label="Epic Games Store"]') ||
      [...document.querySelectorAll("button")]
        .find(btn =>
          btn.getAttribute("aria-label") === "Epic Games Store" ||
...
let gfn = {
  ...
  openFirstTile(title) {
    ...
    const clickTarget =
      ...
      targetCard.querySelector("button, a") ||
      targetCard;
  ...
  async clickEpicTagAndAdd() {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      addBtn = elements.find(b => {
        ...
        return ["MARK AS OWNED", "GET", "+ MARK AS OWNED", "ADD TO LIBRARY"]
  ...
  reportSummary() {
    ...
    if (syncedGames.length) {
      console.log("[GFN] ✅ Synced games:", syncedGames);
    }

    if (skippedGames.length) {
      console.warn("[GFN] ⚠ Skipped games:", skippedGames);
    }

    if (nameMismatches.length) {
      console.warn("[GFN] ⚠ Name mismatches:", nameMismatches);
    }
  }
};
